...
mongoose.connect("mongodb+srv://" + process.env.usernameMongoDB + ":" + process.env.password + "@cluster0.xgjts.mongodb.net/blogitDB", {useNewUrlParser: true, useUnifiedTopology: true});
		...
		if (err) {
			...
			if (blog.length === 0) {
				...
				const Nodejs = new Blog({
					titleInput: _.lowerCase('Node.js'),
				...
				const covid = new Blog({
					titleInput: _.lowerCase('COVID-19'),
...
app.post("/compose", function (req, res) {
	...
	if (newDate.getHours() < 12) {
		const post = new Blog({
			titleInput: _.lowerCase(req.body.titleInput),
